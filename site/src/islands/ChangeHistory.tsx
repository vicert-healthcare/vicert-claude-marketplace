    ...
    <div>
        ...
        {visibleCommits.map((commit) => {
          ...
          return (
            <div key={commit.sha} class="flex items-start gap-3">
              <div class="w-2 h-2 bg-swiss-black mt-1.5 flex-shrink-0" />
              <div class="min-w-0 flex-1">
                <a
                  href={commit.html_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-sm text-swiss-black hover:text-vicert-blue transition-colors block truncate font-medium"
                  title={firstLine}
                >
                  {firstLine}
                </a>
                <div class="flex items-center gap-2 mt-0.5 text-xs text-gray-600">
                  {commit.author && (
                    <span class="font-medium">{commit.author.login}</span>
                  )}
                  <span>{timeAgo(commit.commit.author.date)}</span>
                  <span class="font-mono text-gray-300">{commit.sha.substring(0, 7)}</span>
                </div>
              </div>
            </div>
          );
      ...
      {commits.length > 5 && !expanded && (
        <button
          ...
          class="mt-3 text-xs text-vicert-blue hover:text-vicert-blue-dark font-bold transition-colors"
        >
